"""Shared utility for writing comprehensive trading analysis reports to disk.

This module provides a unified report writing interface used by both the CLI
and the API/Celery worker to ensure consistent output format.
"""
...
def save_report_to_disk(final_state: Dict[str, Any], ticker: str, save_path: Path) -> Path:
    ...
    Creates a structured directory with individual reports from each agent team:
    - 1_analysts/ - Market, sentiment, news, fundamentals analysts
    - 2_research/ - Bull researcher, bear researcher, research manager
    - 3_trading/ - Trader investment plan
    - 4_risk/ - Aggressive, neutral, conservative analysts
    - 5_portfolio/ - Portfolio manager decision
    - complete_report.md - Consolidated report with all sections

    Args:
        final_state: Complete state dictionary from TradingAgentsGraph.propagate()
        ticker: Stock ticker symbol
        save_path: Directory path where reports should be saved

    Returns:
        Path to the complete_report.md file
    """
